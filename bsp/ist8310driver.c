...
uint8_t ist8310_init(void)
{
    ...
    for (writeNum = 0; writeNum < IST8310_WRITE_REG_NUM; writeNum++)//IST8310_WRITE_REG_NUM为需配置寄存器的总数
    {
        ist8310_IIC_write_single_reg(ist8310_write_reg_data_error[writeNum][0], ist8310_write_reg_data_error[writeNum][1]);
		//这个是给IST8310的0x0B寄存器写入0x08开启中断输出就是说当磁数据采集完成时，会将中断引脚拉至低电平，刚好与我们配置的下降沿触发中断呼应然后进入中断回调函数
...
void ist8310_read_mag(fp32 mag[3])
{
    ...
    ist8310_IIC_read_muli_reg(0x03, buf, 6);//从0x03寄存器开始连续读取6个字节的原始数据，存入buf数组
