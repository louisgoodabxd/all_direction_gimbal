...
bool_t bmi088_accel_init(void)
{
    ...
    BMI088_accel_write_single_reg(BMI088_ACC_SOFTRESET, BMI088_ACC_SOFTRESET_VALUE);//对软复位寄存器写入复位值，触发加速度计的软件复位
	//这里的的读操作需要地址与0X80按位与   但是写操作 直接发送原始寄存器地址，因为BMI088的SPI写操作不需要设置额外标志位，传感器会根据通信阶段
	//续：（先地址后数据）自动识别为写操作
