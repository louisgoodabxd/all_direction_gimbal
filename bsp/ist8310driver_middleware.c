...
{
    HAL_I2C_Mem_Read(&hi2c3, IST8310_IIC_ADDRESS <<1, reg,I2C_MEMADD_SIZE_8BIT,buf,len,10);//1：I2C外设句柄 2：从设备的7位/10位地址
	//3：从设备内部寄存器的地址 4：寄存器地址的位宽 5：数据接收缓冲区 用来存放从从设备读取到的原始字节数据 6：要读取的字节总数 7：I2C通讯的超时时间超时后返回错误状态
}
...
{
    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_6, GPIO_PIN_RESET);//IST8310 的 RESET，低电平重启 IST8310
}
